          ...
          {bookmarkState.bookmarks.map((bookmark) => {
            ...
            return (
              <div
                ...
                className="bg-white rounded-2xl shadow-sm border border-[#DBEAFE] hover:shadow-lg transition-all duration-300 cursor-pointer overflow-hidden group relative"
              >
                <button
                  onClick={(e) => handleRemove(e, bookmark.id)}
                  className="absolute top-3 right-3 bg-white/90 backdrop-blur rounded-full p-2 z-10 shadow-sm opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-50 text-slate-400 hover:text-red-500 border border-[#DBEAFE]"
                >
                ...
                <div className="p-5 flex flex-col items-center text-center">
                  <span className="text-xs font-bold text-[#1E3A5F] uppercase tracking-widest mb-1">
                    {vehicle.brand?.name}
                  </span>
