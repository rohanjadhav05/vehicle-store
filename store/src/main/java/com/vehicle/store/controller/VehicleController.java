...
public class VehicleController {
    ...
    public ResponseEntity<ApiResponse<List<Vehicle>>> getVehicles(
        ...
        return ResponseEntity.ok(ApiResponse.success("Vehicles fetched successfully", vehicleService.getVehicles(brandId, fuelTypeId, minPrice, maxPrice)));
    }
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getVehicleSummary() {
        return ResponseEntity.ok(ApiResponse.success("Vehicle summary fetched successfully", vehicleService.getVehicleSummary()));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Get vehicle details", description = "Retrieves detailed information for a specific vehicle by ID")
    public ResponseEntity<ApiResponse<Vehicle>> getVehicleDetails(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.success("Vehicle details fetched successfully", vehicleService.getVehicleDetails(id)));
    }
    ...
    public ResponseEntity<ApiResponse<Vehicle>> createVehicle(@Valid @RequestBody VehicleRequest request) {
        return ResponseEntity.ok(ApiResponse.success("Vehicle created successfully", vehicleService.createVehicle(request)));
    }
